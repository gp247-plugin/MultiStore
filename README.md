...
- **Grant a dedicated administrator per store (Pro):** hand each store to its own administrator (two presets ship: *Store Admin* / *Store Member*); they log in on the store domain, see only their scope, and cannot reach the root admin — **every platform-wide action stays blocked no matter which permissions they hold**.
